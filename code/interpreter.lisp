...
(defclass state ()
  ((style :accessor state-style
          :initform nil
          :initarg :style)))

(defclass frame ()
  ((token-stack :accessor frame-token-stack
                :initform nil)
   (output-sequence :accessor frame-output-sequence
                    :initform (make-array 64 :adjustable t :fill-pointer 0))))

(defclass interpreter ()
  ((level :accessor interpreter-level
          :initform 1)
   (state-stack :accessor interpreter-state-stack
                :initform (list (make-instance 'state)))
   (tokenizer :reader interpreter-tokenizer
              :initform (make-instance 'tokenizer))
   (macros :reader interpreter-macros
           :initform (make-hash-table :test #'equal))
   (frame-stack :accessor interpreter-frame-stack
                :initform (list (make-instance 'frame)))
   (stream-stack :accessor interpreter-stream-stack
                 :initform nil)))
...
(defun evaluate-token (interpreter)
  (with-slots (token-stack output-sequence)
    ...
    (do ((token (pop-token interpreter) (pop-token interpreter)))
        ((null token))
      (cond ((typep token 'control-sequence)
...
(defun evaluate (interpreter)
  (loop while (peek-token interpreter)
        do (evaluate-token interpreter))
  (mapcar #'sanitize (assemble-text (frame-output-sequence (car (interpreter-frame-stack interpreter))))))

(defun push-and-evaluate (interpreter &optional (parameter-type :single))
  (push-frame-stack interpreter parameter-type)
  (unwind-protect (evaluate interpreter)
    (pop-frame-stack interpreter)))
...
(defun evaluate-argument (interpreter)
  (let ((style (interpreter-style interpreter)))
    (setf (interpreter-style interpreter) nil)
    (unwind-protect (car (getf (car (push-and-evaluate interpreter)) :children))
      (setf (interpreter-style interpreter) style))))
